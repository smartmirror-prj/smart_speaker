...
class CK_VISIBLE_PUBLIC CkWebSocketW  : public CkClassWithCallbacksW
{
	...
	// Returns the accumulated received frame data as a string. The StringCharset
	// property indicates the charset (such as "utf-8") that should be used to
	// interpret the received bytes. Calling GetFrameData clears the internal receive
	// buffer.
